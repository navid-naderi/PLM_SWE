...
def create_working_directory(cfg, args):
    ...
    run_id = '{0}_{1}_{2}_{3}'.format(cfg.task["pooling_operation"], cfg.task["num_swe_ref_points"], cfg.task["freeze_swe"], args.seed)
...
def build_downstream_solver(cfg, dataset):
    ...
    if "lr_ratio" in cfg:
        if cfg.lr_ratio > 0:
            ...
            cfg.optimizer.params = [
                {'params': solver.model.mlp.parameters(), 'lr': cfg.optimizer.lr},
                {'params': solver.model.swe_pooling.parameters(), 'lr': cfg.optimizer.lr}
            ]
        optimizer = core.Configurable.load_config_dict(cfg.optimizer)
